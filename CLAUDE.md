...
**Stable release** — input `x.y.z` (e.g. `1.2.0`). Must be strictly greater than current. A human dispatches the single `release` job (dispatch already requires write/admin, so there is no separate approval gate). It validates the version, waits for CI on `main`, updates `package.json` + the install command in `docs/getting-started.md`, and lands the bump on `main` via an **auto-approved `release/v<version>` PR** — opened by the release bot (`aghast-release-review-bot`) and approved by `github-actions[bot]` (auto-approve), two different identities, so it's a reviewed changeset for Scorecard rather than an unreviewed push (**never merged by hand**). Once the PR's checks pass it builds, signs, publishes to npm (`latest` dist-tag), merges the PR, tags `v<version>`, and creates a GitHub Release. Publish-before-merge means a failed publish never advances `main`. The bot must be in `main`'s branch-protection review-bypass list to merge — see [docs/development.md](docs/development.md#releasing).
