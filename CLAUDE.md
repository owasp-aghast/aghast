...
**Stable release** — input `x.y.z` (e.g. `1.2.0`). Must be strictly greater than current. The `prepare` job updates `package.json` + install command in `docs/getting-started.md` and lands the bump on `main` via an **auto-approved `release/v<version>` PR** (bookkeeping — never merged by hand). The `publish-stable` job is gated by the **`release` GitHub Environment**: it pauses until a required reviewer approves the deployment ("Review deployments"), then builds, signs, publishes to npm (`latest` dist-tag), merges the bump PR **only after** npm succeeds, tags `v<version>`, and creates a GitHub Release. Publish-before-merge means a failed publish never advances `main`. The gate needs a one-time `release` environment with a reviewer — see [docs/development.md](docs/development.md#releasing).
