...
| `--discovery <method>` | Discovery method: `opengrep`, `semgrep`, `sarif`, `openant`, `glob`, or `script` (required for `targeted` and `static` types; `sarif`, `openant`, `glob`, and `script` are targeted-only) |
...
- `<id>.yaml` - Opengrep/Semgrep rule file (for checks with `opengrep` or `semgrep` discovery — both tools share the same rule syntax)
- `tests/` - Opengrep/Semgrep rule test files (for checks with `opengrep` or `semgrep` discovery)
